...
## 2025-10-24 - Lazy Load Heavy ML Imports in Streamlit
**Learning:** Top-level imports of heavy ML libraries like `skops` or `scikit-learn` in a Streamlit app introduce significant overhead on every single user interaction. Even if the actual model loading is wrapped in `@st.cache_resource`, merely having the `import skops.io` statement at the top level causes Streamlit to re-evaluate the import and check module caches during every script rerun, adding hundreds of milliseconds to several seconds of lag.
**Action:** Always move heavy, non-UI imports (like `skops.io`) inside the `@st.cache_resource` decorated function that uses them. This ensures the import is truly evaluated only once on the first run, dramatically improving the responsiveness of UI widget interactions.
