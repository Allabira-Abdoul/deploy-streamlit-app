...
# Bolt Optimization: Wrapped inputs in st.form to prevent the app from rerunning
# from top-to-bottom on every single slider movement or selectbox change.
with st.form("employee_prediction_form", border=False):
    ...
    with main_left:
        ...
        with col2:
            with st.container(border=True):
                ...
                role = st.selectbox('JOB ROLE', tuple_maps['JobRole'], index=tuple_maps['JobRole'].index('Sales Representative'))
    ...
    with main_right:
        # Bolt Optimization: Replaced st.button with st.form_submit_button to work with the st.form
        analyze_clicked = st.form_submit_button('Analyze Risk', type="primary", use_container_width=True, help="Calculate the employee's probability of leaving based on the provided profile")
